...
async def cleanup_pass(text: str) -> str:
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            r = await client.post(f"{CLEAN_URL}/chat/completions", json=body)
...
async def discover_clean_model() -> str:
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        r = await client.get(f"{CLEAN_URL}/models")
        r.raise_for_status()
...
async def transcriptions(
    ...
    try:
        ...
        log.info("asr %.2fs total %.2fs clean=%s: %r", t_asr, t_total, do_clean, text[:80])
    finally:
        os.unlink(src.name)
        if wav:
            os.unlink(wav)
